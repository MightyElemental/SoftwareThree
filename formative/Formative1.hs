...
cmbList xs ys = concatMap (\(x,y)->[x,y]) (zip ys xs)
...
sqDiff (x:y:xs) | x > y = (x-y)^2 : sqDiff (y:xs)
...
the1Mk xs = [(name stu, the1 stu) | stu <- xs]
...
tSOF1 xs = [(name stu, sys1 stu, the1 stu) | stu <- xs, sof1 stu > 70]
...
avgSYS1 xs = sysSum / sysTotal
    where
        sysSum = fromIntegral $ sum [sys1 stu | stu <- xs]
        sysTotal = fromIntegral $ length xs
...
`vowelDigit` tests if a `String` has vowels in even positions and digits in odd positions (recall that indexing starts from 0), and as many vowels as digits.
Your solution should satisfy: 
...
vowelDigit xs = (isEven $ length xs) && and [(isOdd num && isDigit l) || (isEven num && isVowel l) | (l, num) <- zip xs [0..] ]
...
{-
## Qxi:[5 mark] [40]
Write a function `toBarcode` that takes a binary string (a string composed of 0s and 1s only) as parameter and returns a string representing a bar-code.
The 0s are transformed into '.' and 1s into '|'. In addition, the function must return Nothing if the string contains a character that is not a 0 or a 1.
...
toBarcode xs | all (`elem` "01") xs = Just $ map conv xs
             | otherwise = Nothing
    where
        conv '0' = '.'
        conv '1' = '|'
