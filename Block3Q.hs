...
instance Semigroup EqTriSym where
  (<>) = (|>) -- associative function is the translation function
instance Monoid EqTriSym where
  mempty = Rotn Nought -- identity does nothing
instance Group EqTriSym where
  ginverse (Rotn ang) = Rotn (neg ang) -- negate the angle and return the rotation
  ginverse refl@(Refl _) = refl -- get the reflect function and return it
...
-- convert output of the function into a monoid so foldMap can accept it
-- then convert back from a monoid to a boolean
...
-}
class Extractable a where -- define Extractable and the extract method generally
  extract :: a -> Bool
instance Extractable Any where -- define extract method specifically for Any
  extract = getAny
instance Extractable All where -- define extract method specifically for All
  extract = getAll
{-
